      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <input placeholder="Generic name" value={form.generic_name as string} onChange={(e) => handleChange('generic_name', e.target.value)} className="px-3 py-2 border rounded" />
            <input placeholder="Manufacturer" value={form.manufacturer as string} onChange={(e) => handleChange('manufacturer', e.target.value)} className="px-3 py-2 border rounded" />
            <input placeholder="Category" value={form.category as string} onChange={(e) => handleChange('category', e.target.value)} className="px-3 py-2 border rounded" />
            <input placeholder="Dosage" value={form.dosage as string} onChange={(e) => handleChange('dosage', e.target.value)} className="px-3 py-2 border rounded" />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <input placeholder="Unit price" value={form.unit_price as string} onChange={(e) => handleChange('unit_price', e.target.value)} className="px-3 py-2 border rounded" />
            <input type="number" placeholder="Stock quantity" value={String(form.stock_quantity)} onChange={(e) => handleChange('stock_quantity', Number(e.target.value || 0))} className="px-3 py-2 border rounded" />
            <input type="number" placeholder="Minimum stock" value={String(form.minimum_stock)} onChange={(e) => handleChange('minimum_stock', Number(e.target.value || 0))} className="px-3 py-2 border rounded" />
          </div>
