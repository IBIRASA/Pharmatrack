...
import ContactPharmacyModal from '../../components/modals/ContactPharmacyModal';
...
export default function PatientActionCards({
  onSearchMedicine,
  onFindNearby,
  pharmacy = null,
}: PatientActionCardsProps) {
  const { t } = useTranslation();
  const [contactModalOpen, setContactModalOpen] = useState(false);
  const [contactPharmacy, setContactPharmacy] = useState<any | null>(null);

  function openContactModal(ph: any) {
    if (!ph) return;
    ...
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <button
          ...
          className="bg-white rounded-xl shadow-sm hover:shadow-xl transition-all p-8 text-left border border-gray-200 hover:border-green-500 group"
        >
        ...
        <button
          ...
          className="bg-white rounded-xl shadow-sm hover:shadow-xl transition-all p-8 text-left border border-gray-200 hover:border-blue-500 group"
        >
          <div className="flex items-start gap-4">
